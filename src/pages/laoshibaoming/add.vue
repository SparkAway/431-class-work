    ...
    >
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="竞赛时间" prop="jingsaishijian">
            <el-input v-model="ruleForm.jingsaishijian" 
                placeholder="竞赛时间" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}'  label="报名人数" prop="baomingrenshu">
            <el-select v-model="ruleForm.baomingrenshu" placeholder="请选择报名人数"  >
              <el-option
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="报名时间" prop="baomingshijian" >
              <el-date-picker
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.laoshixingming!=''&&json.laoshixingming) || json.laoshixingming==0){
                this.ruleForm.laoshixingming = json.laoshixingming
            }
            if((json.suoshuxueyuan!=''&&json.suoshuxueyuan) || json.suoshuxueyuan==0){
                this.ruleForm.suoshuxueyuan = json.suoshuxueyuan
            }
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('laoshibaoming/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('laoshibaoming/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(table+`/update`,obj).then(res => {});
                          this.$http.post('laoshibaoming/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
