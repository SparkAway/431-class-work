...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"20px 10px","margin":"0px auto 0","borderColor":"#00adb5","alignItems":"center","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"#f5f5f5","borderWidth":"0px 0 0px 0","width":"100%","borderStyle":"dashed","height":"auto","order":"2"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>老师姓名</div>
        <el-input v-model="formSearch.laoshixingming" placeholder="老师姓名" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>参赛进度</div>
        <el-select v-model="formSearch.cansaijindu" placeholder="请选择参赛进度" :clearable="true">
          <el-option v-for="(item, index) in cansaijinduOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('laoshidaka','新增')" :style='{"cursor":"pointer","border":"0px solid #db961f","padding":"0px 15px","boxShadow":"0px 0px 0px #f8a412","margin":"0px 10px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"linear-gradient(320deg, rgba(61,199,196,1) 0%, rgba(152,247,245,1) 80%, rgba(61,199,196,1) 100%),rgb(61,199,196)","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/laoshidakaAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
  ...
  export default {
    ...
    created() {
      ...
      this.baseUrl = this.$config.baseUrl;
	    this.cansaijinduOptions = '未开始,进行中,已完成'.split(',');
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.laoshixingming != '') searchWhere.laoshixingming = '%' + this.formSearch.laoshixingming + '%';
        if (this.formSearch.cansaijindu != '') searchWhere.cansaijindu = this.formSearch.cansaijindu;
        this.$http.get('laoshidaka/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/laoshidakaDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
