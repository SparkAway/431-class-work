...
<div :style='{"width":"calc(100% - 40px)","padding":"30px 20px 40px","margin":"20px auto 0","position":"relative","background":"rgba(255,255,255,.3)"}'>
    ...
    >
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}'  label="参赛进度" prop="cansaijindu">
            <el-select v-model="ruleForm.cansaijindu" placeholder="请选择参赛进度"  >
              <el-option
                  v-for="(item,index) in cansaijinduOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="打卡时间" prop="dakashijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="进度内容" prop="jinduneirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='cansaijindu'){
              this.ruleForm.cansaijindu = obj[o];
              this.ro.cansaijindu = true;
              continue;
            }
            if(o=='jinduneirong'){
              this.ruleForm.jinduneirong = obj[o];
              this.ro.jinduneirong = true;
              continue;
            }
            if(o=='dakashijian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('laoshidaka/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('laoshidaka/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
